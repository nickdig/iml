...
function paste(element, targetClasses = []){
	...
	if (type.localeCompare("iml.Class")==0){
		...
		centerAndResizeAttributeViewsOnClass(copyImlClass);
		...
		for (var i = 0; i < targetClasses.length; i++){
			...
			centerAndResizeAttributeViewsOnClass(targetClass);
	...
	for (var i = 0; i < highlightedElement.length; i++){
		...
		if (isCut){
			...
			if (highlightedElement[i].model.attributes.type.localeCompare("iml.Class")==0){
				...
				centerAndResizeAttributeViewsOnClass(imlClass);
			}
		}
	}
}
	...
	if(curAction){
		if (curAction.actionType.localeCompare(ImlActionType.ADD)==0){
			...
			if(curAction.actionTargetType.localeCompare(ImlActionTargetType.CLASS)==0){
				...
				centerAndResizeAttributeViewsOnClass(curAction.previousValue.imlClass);
				highlightedElement.push(paper.findViewByModel(graph.getCell(curAction.actionTarget.id)));
			}
			else if(curAction.actionTargetType.localeCompare(ImlActionTargetType.RELATION)==0){
			...
			if (curAction.actionTargetType.localeCompare(ImlActionTargetType.CLASS)==0){
				...
				var resetVertices = curAction.previousValue;
...
function determineEditMode(){
	swal({
	  ...
	  text: "Would you like to create/edit a new model conforming to the IML Defintion or a User Defined Meta-Model?",
	  icon: "info",
	  buttons: {
		    IML: {
			  text: "IML Definition",
...
function createInstance(){
	if (graph.getElements().length == 0){
		swal({title: "Empty Meta-Model",
			  text: 'Current IML model does not contain any elements; you cannot create an instance from an emtpty meta-model.',
			  icon: "info",
		});
	}
	else{
		if (!instanceEditor){
			swal({
			  title: "Proceed?",
			  text: 'By switching to the instance editor view, all unsaved progress on your model will be lost. Do you wish to continue?',
			  icon: "info",
			  buttons: {
					Confirm: {
					  text: "Yes",
				...
				if (isConfirm) {
					targetMetaModel = imlStructuralModel;
					instanceEditor = true;
					imlStructuralModel = new ImlStructuralModel();
					addPaletteIcons();
					$("#check").removeClass("disabled").find("a").attr("onclick", "reportConformance()");
...
function findNewPos(evt, element){
...
function implementPaper() {
		...
		'element:pointerup': (elementView, evt) => {
			...
			for (var index = 0; index < highlightedElement.length; index++){
				...
				if (overlap(elementsUnder)) {
					...
					const { x, y } = findNewPos(evt, element);
    ...
    paper.on('blank:pointermove', function(evt, x, y) {
        ...
        $('#paperHolder').css('cursor', 'grabbing');
	...
	try{
		...
		if(startVertices.length != endVertices.length){
			undoStack.push(new ImlAction(ImlActionType.MOVE, imlRelationView, ImlActionTargetType.RELATION, startVertices, endVertices));
		}
		if (imlRelation){
...
function generateCode() {
	// if(!instanceEditor) { // May need to add a check for structural vs. behavorial
	// 	var classFiles;
	// 	classFiles = generateStructuralCode(imlStructuralModel);
	...
	// 	});
	// } else {
	// 	generateInstanceModelCode(imlStructuralModel, targetMetaModel);
	// }
	var classFiles;
    ...
    if(instanceEditor){
		...
		conformClasses.forEach(curClass =>{
			...
			if(curClass.isAbstract.toString().localeCompare("false")==0){
				html += '<td class="tg-xwyw"><img id="uc:' + curClass.name + '" height="70%" class="paletteIcon" draggeable="true" ondragstart="drag(this, event)" onclick="addUserClassModelToGraph(200,150,this)" src="Resources/ClassModelIcon.svg"><br>'+curClass.name+'</td>';
				colCount++;
			}
		});
		...
		conformRels.forEach(curRel =>{
			...
			if(curRel instanceof ImlComposition) {
				html += '<td class="tg-xwyw"><img id="com:' + curRel.name + '" width="80%" class="paletteIcon" draggeable="true" ondragstart="drag(this, event)" onclick="addUserRelationToGraphClickEvent(this)" src="Resources/compositionArrowIcon.svg"><br>'+curRel.name+'</td>';
				colCount++;
			}
			else if(curRel instanceof ImlReference) {
				html += '<td class="tg-xwyw"><img id="ref:' + curRel.name + '" width="80%" class="paletteIcon" draggeable="true" ondragstart="drag(this, event)" onclick="addUserRelationToGraphClickEvent(this)" src="Resources/referenceArrowIcon.svg"><br>'+curRel.name+'</td>';
				colCount++;
			}
		});
		...
		html += '<table class="tg">';
		html += '<tr>';
		html += '<td class="tg-xwyw"><img id="classIcon" height="80%" class="paletteIcon" draggeable="true" ondragstart="drag(this, event)" onclick="addClassModelToGraph()" src="Resources/ClassModelIcon.svg"><br>Class</td>';
		html += '<td class="tg-xwyw"><img id="attributeIcon" height="40%" class="paletteIcon" draggeable="true" ondragstart="drag(this, event)" onclick="addAttributeToGraphClickEvent(this)" src="Resources/attributeIcon.png"><br>Attribute</td>';
		html += '<td></td>';
		html += '</tr>';
		html += '<tr>';
		html += '<td class="tg-xwyw"><img id="inheritanceIcon" width="60%" class="paletteIcon" draggeable="true" ondragstart="drag(this, event)" onclick="addRelationToGraphClickEvent(this)" src="Resources/InheritanceArrowIcon.svg"><br>Inheritance</td>';
		html += '<td class="tg-xwyw"><img id="compositionIcon" width="60%" class="paletteIcon" draggeable="true" ondragstart="drag(this, event)" onclick="addRelationToGraphClickEvent(this)" src="Resources/compositionArrowIcon.svg"><br>Composition</td>';
		html += '<td class="tg-xwyw"><img id="referenceIcon" width="60%" class="paletteIcon" draggeable="true" ondragstart="drag(this, event)" onclick="addRelationToGraphClickEvent(this)" src="Resources/referenceArrowIcon.svg"><br>Reference</td>';
		html += '</tr>';
		html += '</table>';
	}
...
function addUserClassModelToGraph(dropPositionX, dropPositionY, imlClassName) {    
	...
	var inheritedAttrs = getInheritedAttributes(targetClass);
...
function getInheritedAttributes(imlClass){
	...
	if(inheritanceRels.size < 1){
		...
		return new Map(imlClass.attributes);
	}
		...
		var recursiveAttrs = getInheritedAttributes(inheritedClass);
		inheritedAttrs = new Map(imlClass.attributes);
		
		return mergeAttributeMapsWithOverload(recursiveAttrs,inheritedAttrs);
	}
}
...
function centerAndResizeAttributeViewsOnClass(imlClass,model=imlStructuralModel) {
	...
	var maxAttributeNameWidth = getMaxAttributeNameWidth(imlClass) + iconPad;
...
function exportCurrentModel() {
    exportStructuralModel(imlStructuralModel, graph);
}
...
function convertValue(imlAttribute, tableCellValueHolder, newValue){
	...
	for (var i = 0; i < oldValue.length; i++){
		...
		if (newValue == ImlType.INTEGER){
			...
			var temp = curVal.toLowerCase();
			var tempNum = parseFloat(temp);
			if (temp == "t" || temp == "true" || (!isNaN(tempNum) && tempNum > 0))
				tempResult.push("TRUE");
			else if (temp == "f" || temp == "false" || (!isNaN(tempNum) && tempNum <= 0))
...
function saveEdit(inputElement) {
	...
	try{
		...
		if (imlClass){ //if the class name changes, ensure the new name is unique
			...
			if (propertyName.localeCompare("Value")==0) { //new value, we need type
				...
				if((Array.isArray(newValue))){
					...
					for (var index = 0; index < newValue.length; index++) { 
						...
						if (!(validAssignment(newValue[index],imlAttribute.type))){
					...
					if (!(newValue.localeCompare("")==0)&&(!(validAssignment(newValue,imlAttribute.type))))
			...
			if (propertyName.localeCompare("ModelName")==0){
				if (validIdentifier(newValue)){
					...
					throw new Error('"' + newValue + '" is not a valid Model name; valid identifiers contain only: letters, digits, underscores, or dollar signs, and must not begin with a digit. Keywords must also not be used.');
				}
			} 
		...
		if(imlClass) {
			...
			centerAndResizeAttributeViewsOnClass(imlClassModel);
		} else if(imlRelation) {
