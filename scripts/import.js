...
function readMetaModel(importFileElement) {
    ...
    reader.onload = function() {
        var fileType = inputFile.name.split('.')[1];
        if(fileType.localeCompare("iml") == 0) {
            importModel(reader.result, inputFile.name, true, graph);
        } else {
            window.alert('File uploads must be of type ".iml"');	  
        }	
    }
	...
	imlStructuralModel.setConformsTo(inputFile.name);
}
    ...
    try {
        ...
        if($structuralModelXml) { // Check for a structural model in the iml file
			...
			if(isMeta){
				...
				if(inputModel.conformsTo.localeCompare("IML Definition")==0){
					instanceEditor = false;
					...
					instanceEditor = true;
					swal({
					  title: "Load Target Meta-Model",
					  text: 'Please load user defined meta-model: ' + inputModel.conformsTo,
					  icon: "info",
					  buttons: {
							Confirm: {
							  text: "OK",
