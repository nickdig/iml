...
function exportStructuralModel(imlModel, graph) {
    ...
    try {
        ...
        imlModel.classes.forEach(imlClass => {
            ...
            xml += ' x="'           + imlClassView.position().x                     + '"';
            xml += ' y="'           + imlClassView.position().y                     + '"';
			xml += ' id="'          + imlClass.id                     				+ '"';
        ...
        document.getElementById('export').setAttribute('href', window.URL.createObjectURL(file));
